...
.. Note::
   ...
   :abbr:`DMARC (Domain-based Message Authentication, Reporting, & Conformance)` set up on the
   :abbr:`DNS (Domain Name System)` will improve deliverability.
   - :doc:`email_domain`
...
.. note::
   ...
   parameter can be set using the `from_filter` field on `ir.mail_server`. For more information on
   the `odoo-bin` see: :doc:`../../../developer/cli`
...
* **Reply messages** of messages sent from Odoo are routed to their original discussion thread (and
  ...
  catchall alias (catchall@example.odoo.com). The catchall address, however, does not have another
...
* **Bounced messages** are used as a Return-Path. One example this is especially
  useful for is in `Odoo Email Marketing <https://www.odoo.com/page/email-marketing>`__. In this case
  bounces are opt-out based on if the email bounced too many times (5) in the last month and the
  bounces are separated by one week. This is done to avoid blacklisting someone because of a mail
  server error. If these circumstances are met then the email is considered invalid and is
...
* Set the :guilabel:`Alias Domain` name in :menuselection:`Settings --> General Settings -->
  Discuss`. Changing the :guilabel:`Alias Domain` will change the catchall's domain for the database.
* If the database's hosting type is Odoo on-premise, create an :guilabel:`Incoming Mail Server` in
  ...
  --> Custom Mail Servers --> Incoming Mail Servers --> New` Fill out the form according to the email
  provider's settings. Leave the :guilabel:`Actions to Perform on Incoming Mails` field blank. Once
  all the information has been filled out, click on :guilabel:`TEST & CONFIRM`.
...
configuration view: :menuselection:`Settings --> Technical Menu --> Email --> Aliases``
